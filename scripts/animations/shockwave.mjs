// Queen shockwave: void gathering → ring expansion → crown flash → max ring → glow dissipation
// 8 frames, variable durations [50, 60, 35, 55, 75, 90, 110, 130]ms = 605ms total

function frame0(ctx, cx, cy) {
  // Void gathering: purple core forming, faint ring hint
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
  ctx.fillStyle = rg(ctx, cx, cy, 0, 20, [
    [0, 'rgba(180,100,255,0.45)'],
    [0.5, 'rgba(120,50,200,0.20)'],
    [1, 'rgba(80,0,160,0)']
  ]);
  ctx.beginPath(); ctx.arc(cx, cy, 20, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
  ctx.strokeStyle = 'rgba(160,80,255,0.40)';
  ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.arc(cx, cy, 22, 0, Math.PI * 2); ctx.stroke();
}

function frame1(ctx, cx, cy) {
  // Ring expanding: bright edge forming, first crown spikes
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
...
function frame2(ctx, cx, cy) {
  // IMPACT flash: white-purple center burst + ring at mid-radius (shortest frame)
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
...
function frame3(ctx, cx, cy) {
  // Crown fully out: ring expanding, spikes long, void spreading
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
...
function frame4(ctx, cx, cy) {
  // Max expansion: large ring + outer halo, spikes fading
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
...
function frame5(ctx, cx, cy) {
  // Dissipation: ring fading, particle wisps
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
